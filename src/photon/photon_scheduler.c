...
const char **parse_command(const char *command) {
  ...
  const char **command_args = (const char **) malloc((num_args + 1) * sizeof(const char *));
...
LocalSchedulerState *LocalSchedulerState_init(
    ...
    int num_workers) {
  LocalSchedulerState *state = (LocalSchedulerState *) malloc(sizeof(LocalSchedulerState));
...
void new_client_connection(event_loop *loop,
   ...
   * scheduler state. */
  LocalSchedulerClient *worker = (LocalSchedulerClient *) malloc(sizeof(LocalSchedulerClient));
