for host in $(cat workers.txt); do
  ...
  ssh -o "StrictHostKeyChecking no" $host 'uptime && echo "/home/ubuntu/core.%p" | sudo tee /proc/sys/kernel/core_pattern'
...
parallel-ssh -h workers.txt  -P -t 0 -I < ~/enable_hugepages.sh
