...
parallel-ssh -t 0 -i -P -H "$WORKER_IPS" -O "StrictHostKeyChecking=no" -I 'bash -s - '$HEAD_IP '$PSSH_NODENUM' $NUM_RAYLETS $LINEAGE_POLICY $MAX_LINEAGE_SIZE $GCS_DELAY_MS < start_worker.sh
...
parallel-ssh -t 0 -i -P -H "$WORKER_IPS" -O "StrictHostKeyChecking=no" 'taskset -p -c 0,1 `pgrep raylet`'
